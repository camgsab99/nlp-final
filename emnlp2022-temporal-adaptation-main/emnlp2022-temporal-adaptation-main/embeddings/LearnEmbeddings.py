...
def preprocess_folder(folder, output_folder, setting):
    ...
    for dataset in datasets:
        ...
        with open(os.path.join(output_folder, dataset.split("/")[-1] + "-all.txt"), "w", encoding="utf-8") as output_file:
            output_file.writelines([l + "\n" for l in df[text_column].tolist()]) # 'text' for clex/stowe
                

        # with open(os.path.join(output_folder, dataset.split("/")[-1] + "-dated.txt"), "w", encoding="utf-8") as output_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", type=str, default="./pretraining_data/clex/controlled/")
    parser.add_argument("--logging_dir", type=str, default="./logs")
    parser.add_argument("--setting", type=str, default="controlled")
