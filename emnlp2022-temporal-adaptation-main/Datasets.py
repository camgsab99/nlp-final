...
def load(config, max_length=64):
    ...
    train_dataset = Dataset.from_pandas(train_data, features=features).map(
    ...
    validation_dataset = Dataset.from_pandas(validation_data, features=features).map(
        lambda ex: config["tokenizer"](ex['text'], max_length=max_length, truncation=True, padding='max_length'), batched=True, batch_size=64)
    ...
    test_dataset = Dataset.from_pandas(test_data, features=features).map(
        lambda ex: config["tokenizer"](ex['text'], max_length=max_length, truncation=True, padding='max_length'), batched=True, batch_size=64)
...
def load_progressive(config, max_length=64):
    ...
    for i in range(10):
        ...
        dataset = Dataset.from_pandas(data, features=features).map(
